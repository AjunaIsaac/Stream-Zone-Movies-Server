...
const formatPhoneNumber = (phone) => {
  ...
  // Handle local formats (070, 075, etc.)
  if (/^(07[0-9])\d{7}$/.test(trimmed)) {
    return '256' + trimmed.slice(1);
  }
  
  // Handle international format with +256
  if (/^\+256(7[0-9])\d{7}$/.test(trimmed)) {
    return trimmed.slice(1);
  }
  
  // Handle international format without + (256)
  if (/^256(7[0-9])\d{7}$/.test(trimmed)) {
    return trimmed;
  }
  
  return trimmed; // return as-is if doesn't match any pattern
};
...
const validatePaymentRequest = (body) => {
  ...
  if (!/^256(70|73|74|75|76|77|78|79)\d{7}$/.test(formattedPhone) &&
    !/^07[0-9]\d{7}$/.test(phone.trim())) {
    return {
      valid: false,
      message: "Invalid phone format. Must be a valid Ugandan number (e.g., 0751234567 or 256751234567)"
    };
  }
...
app.post('/pay', async (req, res) => {
  ...
  try {
    ...
    const statusCode = error.code === 'ECONNABORTED' ? 408 : (error.response?.status || 500);
